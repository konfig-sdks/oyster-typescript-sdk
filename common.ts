...
import { readableStreamToString, OysterError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OysterError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OysterError(e, e.response?.data)
                }
                if (innerError instanceof OysterError) {
                    // Got "OysterError" from the above try block
                    throw innerError;
                }
